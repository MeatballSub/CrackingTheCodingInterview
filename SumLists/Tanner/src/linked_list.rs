...
impl<T> Node<T>
{
    fn new(element: T) -> Self { Node { next: None, element } }

    fn into_element(self: Box<Self>) -> T { self.element }
}
...
impl<T> LinkedList<T>
{
    ...
    pub fn push_front(&mut self, element: T) { let _ = self.push_front_mut(element); }

    pub fn push_back(&mut self, element: T) { let _ = self.push_back_mut(element); }

    pub fn pop_front(&mut self) -> Option<T> { self.pop_front_node().map(Node::into_element) }

    pub fn head_node(&self) -> Option<std::ptr::NonNull<Node<T>>> { self.head }
    ...
    pub fn push_front_mut(&mut self, element: T) -> &mut T
    {
        let mut node = unsafe { std::ptr::NonNull::new_unchecked(Box::into_raw(Box::new(Node::new(element)))) };
        unsafe {
    ...
    #[inline]
    unsafe fn push_front_node(&mut self, node: std::ptr::NonNull<Node<T>>)
    {
        unsafe {
    ...
    {
        UnsafeCursor { list_head: &mut self.head,
    ...
    {
        UnsafeCursorMut { list_head: &mut self.head,
                          list_tail: &mut self.tail,
                          list_len: &mut self.len,
                          current: self.head,
                          previous: None }
...
impl<T: Clone> Clone for LinkedList<T>
{
    fn clone(&self) -> Self
    {
        ...
        self.iter().for_each(|element| list.push_back(element.clone()));
        list
    }
}
...
impl<T: PartialEq> PartialEq for LinkedList<T>
{
    ...
    fn ne(&self, other: &Self) -> bool { self.len() != other.len() || self.iter().ne(other) }
}
...
{
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result { f.debug_tuple("IntoIter").field(&self.list).finish() }
}
...
{
    #[must_use]
    pub fn current(&mut self) -> Option<T> { unsafe { self.current.map(|current| (*current.as_ptr()).element.clone()) } }
...
{
    #[must_use]
    pub fn current(&mut self) -> Option<&mut T> { unsafe { self.current.map(|current| &mut (*current.as_ptr()).element) } }
